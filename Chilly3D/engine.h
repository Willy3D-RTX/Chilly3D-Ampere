...
public:
	void setup(C3D_Setup_t _setup, int _value)
	{
		...
		{
			message.engine(__FILE__, __LINE__, "Opciones del motor: DEBUG=%d", _value);
			root.flags.debug = (bool)_value;
			return;
		}
		if (_setup == C3D_SETUP_ANSI)
		{
			message.engine(__FILE__, __LINE__, "Opciones del motor: ANSI=%d", _value);
			root.flags.ansi = (bool)_value;
			return;
		}
		if (_setup == C3D_SETUP_WIDTH)
		{
			message.engine(__FILE__, __LINE__, "Opciones del motor: VIDEO.WIDTH=%d", _value);
			root.video.width = _value;
			return;
		}
		if (_setup == C3D_SETUP_HEIGHT)
		{
			message.engine(__FILE__, __LINE__, "Opciones del motor: VIDEO.HEIGHT=%d", _value);
			root.video.height = _value;
			return;
		}
		if (_setup == C3D_SETUP_FULLSCREEN)
		{
			message.engine(__FILE__, __LINE__, "Opciones del motor: VIDEO.FULLSCREEN=%d", _value);
			root.video.fullscreen = (bool)_value;
			return;
		}
		if (_setup == C3D_SETUP_DOBLEBUFFER)
		{
			message.engine(__FILE__, __LINE__, "Opciones del motor: VIDEO.DOBLEBUFFER=%d", _value);
	...
	void init(uint16_t _width, uint16_t _height)
	{
		...
		{
			message.error(__FILE__, __LINE__, "Error en la resolucion de pantalla");
			_width = C3D_VIDEO_WIDTH_MIN;
			_height = C3D_VIDEO_HEIGHT_MIN;
			message.info(__FILE__, __LINE__, "Ajustando: VIDEO.MODE=%dx%d", _width, _height);
		}
	...
	void dispose(void)
	{
		message.engine(__FILE__, __LINE__, "Cerrando %s..", C3D_API_TITLE);
		hal_video_dispose();
		message.dispose();
		message.engine(__FILE__, __LINE__, "%s se ha finalizado con exito!", C3D_API_TITLE);
	}
};
